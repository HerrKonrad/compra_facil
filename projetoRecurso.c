...
int Temp_EliminarProduto(PRODUTO produto)
{
    ...
    while (!feof(f))
    {
        STRING *V = splitLine(f, n_campos_max, &n_campos_lidos, ";");
        ++n_linhas_lidas;
    ...
    system("cls");
    printf("\t\t\t%s eliminado corretamente!!", nomeProduto);
    fclose(f);
    return temp;
}
...
{
    PRODUTO *prod = malloc(65536 * sizeof(PRODUTO));
    int n_campos_max = 20;
    int n_campos_lidos;
